...
# Chart 2: simple scatterplot (time series)
png(filename = 'plot2.png', width = 480, height = 480)
plot(df$period, df$Global_active_power, type = 'l', xlab='', ylab = 'Global Active Power (kilowatts)')
dev.off()

# Chart 3: multiple-line scatterplot (time series)
# Note that I am not using variable labels for the legend, to avoid the ugly underscores.
# NB: two ways to produce such a chart: using plot() for the first series and
# annotating with further lines for the other series ...
...
# ... and also using matplot() but correcting for the wrong labelling of the X axis
# (matplot() does not handke POSIXct variables)
